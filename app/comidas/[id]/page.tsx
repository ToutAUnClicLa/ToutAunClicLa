          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {product.productos_relacionados.map((relatedProduct: any) => (
              <Link
                ...
                href={`/comidas/${relatedProduct.id}`}
                className="group"
              >
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        // Verify it's a food product (category_id = 2)
        if (data.categoria_id !== 2) {
          // If not food, redirect to products
        ...
        <div className="container max-w-5xl">
          <div className="flex items-center text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6 overflow-x-auto whitespace-nowrap">
            ...
            {product.subcategorias?.nombre && (
              <>
                <ChevronRight className="h-4 w-4 mx-1 sm:mx-2 flex-shrink-0" />
                <Link href={getRestaurantUrlWithFallback(product.subcategorias.nombre)} className="hover:text-gray-900">
                  {product.subcategorias.nombre}
                </Link>
              </>
            )}
